...
int main(int argc, char** argv) {
    ...
    for (int i = 12; i >= 4; i--) {
        float density = std::pow(2.0, -i);

        auto tensor = make_data(l, c, density);

        for (auto tile_size : grid_sizes) {
            std::cout << "Running for density: " << density << " and tile size: " << tile_size << " on tensor with shape: " << tensor.get_shape_string() << std::endl;
            double time_taken = make_a_run(tensor, "", CoOrdinate({1}), tile_size, true);
